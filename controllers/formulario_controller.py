...
class FormularioController:
    ...
    def create_formulario(user_id, data):
        ...
        if FormularioModel.create_formulario(user_id, nome, email, data_nascimento, cpf, genero):
    ...
    def get_formulario(formulario_id):
        f = FormularioModel.find_by_id(formulario_id)
        if not f:
            return {"error": "Formulário não encontrado"}, 404
        return {"id": f['id'], "user_id": f['user_id'], "nome": f['nome'], "email": f['email'],
                "data_nascimento": f['data_nascimento'], "cpf": f['cpf'], "genero": f['genero']}, 200

    @staticmethod
    def update_formulario(formulario_id, data):
        if FormularioModel.update_formulario(formulario_id, data) is None:
            return {"error": "Formulário não encontrado"}, 404
        return {"message": "Formulário atualizado com sucesso"}, 200

    @staticmethod
    def delete_formulario(formulario_id):
        if FormularioModel.delete_formulario(formulario_id) is None:
            return {"error": "Formulário não encontrado"}, 404
